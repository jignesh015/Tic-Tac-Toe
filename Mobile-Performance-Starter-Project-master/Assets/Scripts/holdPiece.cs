...
public class holdPiece : MonoBehaviour {
		...
		if (GameLogic.GetComponent<GameLogic>().playerTurn == true) {
			if (holdingPiece == true) {
				...
				if (Physics.Raycast(raycastHolder.transform.position, (forwardDir), out hit)) {
					...
					pieceBeingHeld.transform.position = Vector3.Lerp (pieceBeingHeld.transform.position, gravityAttractor.transform.position, speed);
	...
	public void selectGridPlate (GameObject selectedPlate) {
		if (holdingPiece == true) {
			pieceBeingHeld.transform.position = Vector3.Lerp (pieceBeingHeld.transform.position, selectedPlate.transform.position, speed);
			holdingPiece = false;
		}
	}
